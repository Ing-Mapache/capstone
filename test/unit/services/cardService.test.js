...
describe('Card Service', () => {
  ...
  describe('initializeDeck', () => {
    it('debería inicializar la baraja para un juego', async () => {
      const gameId = 'uuid-game';
      gameRepository.getGameById.mockResolvedValue({ id: gameId });
      cardRepository.initializeDeck.mockResolvedValue([{ id: 'uuid-card', color: 'red', value: '5' }]);

      const result = await cardService.initializeDeck(gameId);

      expect(gameRepository.getGameById).toHaveBeenCalledWith(gameId);
      expect(cardRepository.initializeDeck).toHaveBeenCalledWith(gameId);
    ...
    it('debería lanzar un error si el juego no existe', async () => {
      ...
      await expect(cardService.initializeDeck(gameId)).rejects.toThrow('El juego especificado no existe');
    });
  });
});